...
@ApplicationScoped
class AppEditListingIconUploadedProcessor(
    ...
    fun onStart(@Observes startupEvent: StartupEvent) {
        val config = config.objectStorageNotifications().appEditListingIconUploads()
    ...
    private fun processMessage(message: PubsubMessage, consumer: AckReplyConsumer) {
        ...
        QuarkusTransaction.disallowingExisting().call {
            ...
            val job = AppEditListingIconUploadJob.findByBucketIdAndObjectId(bucketId, objectId) ?: run {
    ...
    private fun processEvent(event: ObjectUploadEvent, job: AppEditListingIconUploadJob) {
        if (job.appEditListing.appEdit.submitted) {
            job.backgroundOperation.result = ConsoleApiError(
        ...
        val newIcon = Image(
            ...
            .also { it.persist() }
        val existingIcon = job.appEditListing.icon
        if (existingIcon == null) {
            job.appEditListing.iconImageId = newIcon.id
        } else {
        ...
        job.backgroundOperation.result = UploadAppEditListingIconResult
            .getDefaultInstance()
            .toByteArray()
